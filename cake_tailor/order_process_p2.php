<?php
    ...
    $filtered = array(
        ...
        'o_lettering' => mysqli_real_escape_string($link, $_POST['o_lettering']),
        'o_lcolor' => mysqli_real_escape_string($link, $_POST['o_lcolor']),
        'o_cream' => mysqli_real_escape_string($link, $_POST['o_cream'])
    );
    $o_date = date("Y-m-d H:i:s");

    if ($filtered['o_name'] and $filtered['o_hp'] and $filtered['o_pickupD']  and $filtered['o_pickupT'] and $filtered['o_quantity'] and $filtered['o_size'] and $filtered['o_shape'] and $filtered['o_flavor'] and $filtered['o_scolor'] and $o_date) {
        $query = "
            INSERT INTO orderL (
                o_name,
                o_hp,
    ...
    if ($result == false) { ?>
        <script>      
            ...
            location.replace("./3d_custom2.html");
        </script>
    <?php
